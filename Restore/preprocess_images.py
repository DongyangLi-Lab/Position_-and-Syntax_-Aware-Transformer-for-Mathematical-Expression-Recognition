...
import sys, os, argparse, json, glob, logging, pathlib
import numpy as np
from PIL import Image

sys.path.insert(0, '%s' % os.path.join(os.path.dirname(__file__), '../tools/'))
from Restore.image_utils import *
...
def main_parallel(filename, postfix, output_folder, crop_blank_default_size, pad_size, buckets, downsample_ratio):
    ...
    if not status:
        logging.info(
            '%s (after cropping and padding) is larger than the largest provided bucket size, left unchanged!' % filename)
...
def main(args):
    ...
    results = pool.map(main_parallel, [(filename, postfix, os.path.join(output_dir, os.path.basename(filename)),
                                        crop_blank_default_size, pad_size, buckets, downsample_ratio) for filename in
                                       filenames])
    pool.close()
    pool.join()
